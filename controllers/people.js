...
/**
 * Route to list people records
 * @route {GET} /people/index/{fieldname}/{fieldvalue}
...
router.get(['/', '/index/:fieldname?/:fieldvalue?'], async (req, res) => {  
	try{
		...
		if(search){
			let searchFields = People.searchFields();
		...
		query.order = People.getOrderBy(req);
		query.attributes = People.listFields();
		let page = parseInt(req.query.page) || 1;
		let limit = parseInt(req.query.limit) || 20;
		let result = await People.paginate(query, page, limit);
...
/**
 * Route to view People record
 * @route {GET} /people/view/{recid}
 * @param {array} path - Array of express paths
 * @param {callback} middleware - Express middleware.
 */
router.get(['/view/:recid'], async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		query.attributes = People.viewFields();
		let record = await People.findOne(query);
		if(!record){
			return res.notFound();
		}
...
, async function (req, res) {
	try{
		...
		//save People record
		let record = await People.create(modeldata);
...
/**
 * Route to get  People record for edit
 * @route {GET} /people/edit/{recid}
 * @param {string} path - Express path
 * @param {callback} middleware - Express middleware.
 */
router.get('/edit/:recid', async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		query.attributes = People.editFields();
		let record = await People.findOne(query);
		if(!record){
			return res.notFound();
		}
...
/**
 * Route to update  People record
 * @route {POST} /people/edit/{recid}
 * @param {string} path - Express path
 * @param {callback} middleware - Express middleware.
 */
...
, async (req, res) => {
	try{
		...
		await People.update(modeldata, {where: where});
...
/**
 * Route to delete People record by table primary key
 * Multi delete supported by recid separated by comma(,)
 * @route {GET} /people/delete/{recid}
 * @param {array} path - Array of express paths
 * @param {callback} middleware - Express middleware.
 */
router.get('/delete/:recid', async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		let records = await People.findAll(query);
		records.forEach(async (record) => { 
			//perform action on each record before delete
		});
		await People.destroy(query);
		return res.ok(recid);
	}
	catch(err){
		return res.serverError(err);
